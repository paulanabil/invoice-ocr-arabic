...
def extract(name: str):
    ...
    cfg = _settings()
    model_path = frappe.get_site_path(cfg.current_model_path)
    ...
    # Parse items from text (heuristic)
    rows = []
    parsed = parse_items(raw)
    # Attach line crops to child rows for future training
...
def learn_from_rows(name: str):
    """Store corrected snippets as training samples."""
    doc = frappe.get_doc("OCR Invoice Import", name)
    for row in doc.items or []:
        # Only store if we have an image crop and a corrected text
...
@frappe.whitelist()
def retrain_model():
    """Retrain from base using collected training samples (transfer learning)."""
    cfg = _settings()
    ...
    # Export training samples to temp files expected by kraken.train (placeholder)
    ...
    # Move/point current_model_path to new model
